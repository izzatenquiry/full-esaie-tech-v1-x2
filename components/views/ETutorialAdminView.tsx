...
import { type TutorialContent, type PlatformStatus, type Announcement, type PlatformSystemStatus, type Language } from '../../types';
...
const TutorialManagementPanel: React.FC<{ language: Language }> = ({ language }) => {
  const T = getTranslations(language).eTutorialAdminView;
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-4 p-4 border border-neutral-200 dark:border-neutral-700 rounded-md">
        <h3 className="font-semibold text-lg">{T.mainSection}</h3>
        <div>
          <label htmlFor="mainVideoUrl" className="block text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.mainVideoUrl}</label>
          <input
            id="mainVideoUrl"
            name="mainVideoUrl"
            type="text"
        ...
        <div>
          <label htmlFor="mainTitle" className="block text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.mainTitle}</label>
          <input
            id="mainTitle"
            name="mainTitle"
            type="text"
        ...
        <div>
          <label htmlFor="mainDescription" className="block text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.mainDescription}</label>
          <textarea
            id="mainDescription"
      ...
      <div className="space-y-6">
        <h3 className="font-semibold text-lg">{T.tutorials}</h3>
        {content.tutorials.map((tutorial, index) => (
          <div key={index} className="p-4 border border-neutral-200 dark:border-neutral-700 rounded-md space-y-4">
            <h4 className="font-semibold">{T.tutorial} {index + 1}</h4>
            <div>
              <label htmlFor={`title-${index}`} className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.title}</label>
              <input
                id={`title-${index}`}
                name="title"
                type="text"
            ...
            <div>
              <label htmlFor={`description-${index}`} className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.description}</label>
              <textarea
             ...
             <div>
                <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">{T.thumbnail}</label>
      ...
      <div className="flex items-center gap-4">
        ...
        {saveStatus === 'saved' && <p className="text-sm text-green-600">{T.saveSuccess}</p>}
      </div>
    </div>
  );
};

const PlatformUpdatesPanel: React.FC<{ language: Language }> = ({ language }) => {
    const T = getTranslations(language).eTutorialAdminView;
    ...
    if (!status) return <div>{T.loading}</div>;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="p-4 border border-neutral-200 dark:border-neutral-700 rounded-md space-y-4">
                <h3 className="font-semibold text-lg">{T.statusTitle}</h3>
                <div>
                    <label htmlFor="platform-status" className="block text-sm font-medium mb-1">{T.currentStatus}</label>
                    <select id="platform-status" value={status.status} onChange={(e) => handleStatusChange('status', e.target.value)} className="w-full p-2 border rounded-md bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600">
                        ...
                        <option value="outage">{T.outage}</option>
                    </select>
                </div>
                <div>
                    <label htmlFor="status-message" className="block text-sm font-medium mb-1">{T.statusMessage}</label>
                ...
                <div className="flex items-center gap-4">
                    <button onClick={handleSaveStatus} className="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-700">{T.updateStatus}</button>
                    {saveStatus === 'saved' && <p className="text-sm text-green-600">{T.statusUpdated}</p>}
                </div>
            </div>

            <div className="p-4 border border-neutral-200 dark:border-neutral-700 rounded-md space-y-4">
                <h3 className="font-semibold text-lg">{T.announcementsTitle}</h3>
                <div className="space-y-3 p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-md">
                    <h4 className="font-medium">{T.postNew}</h4>
                    ...
                    <button onClick={handleAddAnnouncement} className="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-700">{T.post}</button>
                </div>
...
const ETutorialAdminView: React.FC<{ language: Language }> = ({ language }) => {
    const [isTutorialsVisible, setIsTutorialsVisible] = useState(false);
    const T = getTranslations(language).eTutorialAdminView;

    return (
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm">
            <PlatformUpdatesPanel language={language} />
            ...
            <div>
              ...
              >
                  <h2 className="text-xl font-semibold group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{T.eTutorialTitle}</h2>
              ...
              <div className={`grid overflow-hidden transition-all duration-500 ease-in-out ${isTutorialsVisible ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0'}`}>
                  <div className="overflow-hidden">
                      <div className="pt-2">
                          ...
                          <TutorialManagementPanel language={language} />
                      </div>
                  </div>
              </div>
            </div>
